import numpy as np
import json
from utils import rotate90, find_partner, check_distance, cal_distance, find_positions, select_dynamic_block, select_dynamic_block_last_two_rows_plgr
                    ...
                    if i < shape - 2:
                        ...
                        if j >= shape-2 and i >= shape-2:
                            if abs(ii-i) == 1 and abs(jj-i) == 1:
                                ...
                                break
                        x_cord,y_cord,size,n_iter = select_dynamic_block_last_two_rows_plgr(j,i,jj,ii)
                        board = rotate90(x_cord,y_cord,size,n_iter,board,ops)
                        ii,jj = find_positions(i,j,board)
                        dist = cal_distance(i,j,ii,jj)
...
if __name__ == "__main__":
    ...
    with open("answer_plgr.json", "w") as f:
        json.dump(ops, f)
    print(new_board)