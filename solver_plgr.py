...
def select_dynamic_block_colums(x1,y1,x2,y2,board):
    ...
    if x1 > x2:
        if dy == 0:
            ...
            if dy == 1:
                x_cord,y_cord,size, n_iterations = (x2,y2,2,2)
            elif dx == 1:
                x_cord,y_cord,size,n_iterations = (x2,y2,2,2)
            else:
                if dy < abs(x1-1-x2):
                    ...
                    x_cord,y_cord,size,n_iterations = (x2,y2,abs(x1-1-x2)+1,2)
        else:
            if dx == dy:
                x_cord,y_cord,size, n_iterations = (x2,y1,dx+1,2)    
            elif dx > dy:
                x_cord,y_cord,size,n_iterations = (x2,y2-dy,dy+1,1)
            else:
                x_cord,y_cord,size,n_iterations = (x2-dx,y2-dx,dx+1,1)     
        ...
        if dx == 0:
            if x1 == shape - 3 and y1 == shape - 2:
                if dy < x1:
                    x_cord,y_cord,size,n_iterations = (x1-dy,y2,dy+1,1)
                else:
                    x_cord, y_cord, size, n_iterations = (x1-1,y2,2,1)
            else:
                if dy == 1:
                    x_cord,y_cord,size,n_iterations = (x2-dy, y1, dy+1, 3)
                else:
                    if dy < x1:
                        x_cord,y_cord,size,n_iterations = (x2-dy,y2-dy,dy+1,3)
                    else:
                        x_cord,y_cord,size,n_iterations = (0, y2-x1,x1+1,3)
        elif dx == 1 and dy == 1:
            x_cord,y_cord,size,n_iterations = (x2-dx, y2, dx+1, 3)
        else:
...
def solver(board, paired, ops):
    ...
    for i in range(2,shape):
        for j in range(shape-1, shape-3, -1):
            ...
            paired[ii][jj] = True
    # r = r+2
    return solver(board[2:,:-2],paired[2:, :-2],ops)
...
if __name__ == "__main__":
    ...
    new_board, ops = solver(board, paired, ops)
    print(f"Number of step: {len(ops)}")
