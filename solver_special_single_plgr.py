...
def all_false_on_edge(local_paired, size):
    """
    TRUE nếu mọi điểm local False của subboard đều nằm trên viền subboard.
    """
    def on_edge(i, j):
        return (i == 0 or j == 0 or i == size-1 or j == size-1)
    ...
    if paired_or_not == False:
        ...
        if all_false_on_edge(local_paired,size):
            ...
            for i in range(size-1):
                if tx + size > shape - 1:
                    break
                if (local_paired[i][size-1] == False) and (local_paired[i+1][size-1] == False) and (board[ty+j][tx+size-1] == board[ty+j][tx+size]) and (board[ty+j+1][tx+size-1] == board[ty+j+1][tx+size]):
                    bx = tx + size - 1
                    by = ty + i
                    board = rotate90(bx, by, 2, 1, board, ops, r=0)
                    paired_global = paired_mask(board, paired_global, shape)
