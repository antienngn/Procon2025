...
def subboard_is_paired_general(board, paired_global, tx, ty, size):
    ...
    if paired_or_not == False:
        ...
        for i in range(size-1):
            ...
            if tx - 1 < 0:
                break
            if (not local_paired[i][0]) and (not local_paired[i+1][0]) and (board[ty+j][tx] == board[ty+j][tx-1]) and (board[ty+j+1][tx] == board[ty+j+1][tx-1]):
                bx = tx
                by = ty + i
                # print(board[by][bx])
        ...
        for i in range(size-1):
            if tx + size > shape - 1:
                break
            if (local_paired[i][size-1] == False) and (local_paired[i+1][size-1] == False) and (board[ty+i][tx+size-1] == board[ty+i][tx+size]) and (board[ty+i+1][tx+size-1] == board[ty+i+1][tx+size]):
                bx = tx + size - 1
                by = ty + i
                board = rotate90(bx, by, 2, 1, board, ops, r=0)
                paired_global = paired_mask(board, paired_global, shape)
...
def rotate_simulator(rotable_cells,board):
    ...
    if not highest_num_pair:
        return 
    if ancestor_board_pairs + 1 < highest_num_pair["number_of_pair"]:
    ...
    elif total_paired == highest_num_pair["number_of_pair"]:
        best_ops["x_cord"] = highest_num_pair["x_cord"]
        best_ops["y_cord"] = highest_num_pair["y_cord"]
        best_ops["size"] = highest_num_pair["size"]
        best_ops["n_iter"] = highest_num_pair["n_iter"]
    ...
    while True:
        for i in range(shape):
            for j in range(shape):
                ...
                paired_checked = paired_mask(board,paired,shape)
                # print(board)
                # print(f"Total pairs {total_pairs} and current pairs{count_pairs(board)}")
