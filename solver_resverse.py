...
def select_dynamic_block(x1,y1,x2,y2,board):
    ...
    if x1 < x2:
        ...
        if dy == 0:
            ...
            if abs(x1-1-x2) <= abs(y2-shape):
                x_cord,y_cord,size,n_iterations = (x2,y2,dx,1)
            else:
                x_cord,y_cord,size,n_iterations = (x2-abs(y2-shape)+1,y2,abs(y2-shape),1) 
        elif dx == 0:
            if dy < abs(x1-shape):
                x_cord,y_cord,size,n_iterations = (x1, y1, dy+1,1)
            else:
            ...
            if abs(xx-x2) == abs(yy-y2):
                x_cord,y_cord,size,n_iterations = (x2,yy,abs(yy-y2)+1,2)
            else:
                if dx == dy:
        ...
        for i in range(shape):
            for j in range(shape-1,-1,-1):
                ...
                if abs(ii - i) == 1 and abs(jj-j) == 0:
                    board = rotate90(j-1, i, 2, 3, board, ops)
                ii, jj = find_positions(i, j, board)
                paired[i][j] = True
                paired[ii][jj] = True
                score += 1
        ...
        for i in range(shape-2, shape):
            for j in range(shape-1, -1, -1):
                ...
                while dist > 1:
                    if j <= 1:
                        if abs(ii-i) == 1 and abs(jj-j) == 1:
                            ...
                            break
                    x_cord,y_cord,size,n_iter = select_dynamic_block_last_two_rows(j,i,jj,ii)
                    board = rotate90(x_cord,y_cord,size,n_iter,board,ops)
                    ii,jj = find_positions(i,j,board)
                    dist = cal_distance(i,j,ii,jj)
                ...
                if abs(ii - i) == 0 and abs(jj-j) == 1:
                    board = rotate90(jj,ii,2, 1,board, ops) 
                ii, jj = find_positions(i, j, board)
                paired[i][j] = True
                paired[ii][jj] = True
                score += 1

                # print("asdads", board, dist, board[i][j], score)
    return board, ops
